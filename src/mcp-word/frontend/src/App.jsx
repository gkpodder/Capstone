...
import VoiceRecorder from "./VoiceInput";   // <── NEW
...
export default function App() {
  ...
  const sendPrompt = async (text) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (data.permissionRequest?.required) {
  ...
  const handlePermissionSubmit = async (values) => {
    setPermissionRequest(null);
    const payload = { __permissionResponse: true, values };
    ...
    await sendPrompt(JSON.stringify(payload));
  };

  const handlePermissionCancel = () => setPermissionRequest(null);
    ...
    <div className="app-root">
      ...
      <main className="app-main">
        <section className="column conversation">
          ...
          {/* 🔊 INPUT BAR WITH VOICE BUTTON ADDED HERE */}
        ...
        <section className="column status">
          <div className="panel">
            <div className="panel-header">
              ...
              {isRunning && <span className="pill pill-running">Agent is working…</span>}
            </div>
              ...
              <div className="thoughts-body">
                ...
                {(!messages.some((m) => m.thoughtProcess) ||
                  ...
                  <div className="empty-thoughts">
                    No thought process yet.
                  </div>
                )}
              </div>
            ) : (
              <div className="thoughts-body thoughts-body-muted">
                Thought process is hidden.
              </div>
            )}
          </div>
        </section>
