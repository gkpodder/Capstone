...
async function closeFirefox() {
  try {
    if (fxBrowser) await fxBrowser.close();
  } catch {}
  fxBrowser = null;
  fxContext = null;
  fxPage = null;
...
function simpleScore(target, candidate) {
  ...
  let hit = 0;
  for (const t of tks) if (cks.has(t)) hit++;
...
async function clickAt(x, y) {
  ...
  await execAsync(
    `${cliclick} m:${Math.round(x)},${Math.round(
      y
...
app.post("/word/open", async (req, res) => {
  ...
  try {
    const script = filePath
      ? `tell application "Microsoft Word" to open POSIX file "${escQuotes(
          filePath
        )}"`
...
// ---- DuckDuckGo search (replaces Google) ----
async function handleSearch(req, res) {
  const query = req.body?.query ?? "";
  try {
    const { fxPage } = await ensureFirefox();
    await fxPage.goto("https://duckduckgo.com/", { waitUntil: "domcontentloaded" });

    const input = fxPage.locator("input[name=q]");
    await input.first().waitFor({ timeout: 15000 });
    await input.first().fill(query);

    await Promise.all([
      fxPage.keyboard.press("Enter"),
      fxPage.waitForLoadState("domcontentloaded"),
    ]);

    res.json({ searched: query, url: fxPage.url(), engine: "duckduckgo" });
  } catch (e) {
    res.status(500).json({ error: String(e?.message || e) });
  }
}

// New generic search endpoint
app.post("/fx/search", handleSearch);

// Backwards-compatible alias (no longer uses google.com)
app.post("/fx/google_search", handleSearch);
...
app.post("/fx/click_text", async (req, res) => {
  ...
  try {
    ...
    try {
      if (role === "link" || !role) {
        await fxPage.getByRole("link", { name }).first().click({ timeout: 1500 });
        return res.json({ clickedBy: "role:link", text: rawText });
      }
    } catch {}
    try {
      if (role === "button" || !role) {
        await fxPage.getByRole("button", { name }).first().click({ timeout: 1500 });
        return res.json({ clickedBy: "role:button", text: rawText });
      }
    } catch {}
    try {
      await fxPage.getByText(name, { exact }).first().click({ timeout: 1500 });
      return res.json({ clickedBy: "getByText", text: rawText });
    } catch {}

    // Fallback: visible clickables, fuzzy
    const candidates = await fxPage.$$(
    ...
    for (let i = 0; i < 3; i++) {
      ...
      try {
        await fxPage.getByRole("link", { name }).first().click({ timeout: 800 });
        return res.json({ clickedBy: "role:link+scroll", text: rawText });
      } catch {}
      try {
        await fxPage.getByRole("button", { name }).first().click({ timeout: 800 });
        return res.json({ clickedBy: "role:button+scroll", text: rawText });
      } catch {}
      try {
        await fxPage.getByText(name, { exact }).first().click({ timeout: 800 });
        return res.json({ clickedBy: "getByText+scroll", text: rawText });
      } catch {}
    }
    res
      .status(404)
      .json({ error: `Could not find a visible element matching "${rawText}"` });
  } catch (e) {
    res.status(500).json({ error: String(e?.message || e) });
  }
});
...
app.post("/fx/scroll", async (req, res) => {
  ...
  try {
    ...
    if (to === "top")
      ...
      return res
        .status(400)
        .json({ error: "Provide { to: 'top'|'bottom' } or { px: number }" });
...
app.post("/fx/type", async (req, res) => {
  ...
  try {
    ...
    await fxPage.waitForSelector(selector, { timeout: 15000 });
    if (clear) {
      await fxPage.click(selector, { clickCount: 3 });
      await fxPage.keyboard.press("Backspace");
    }
...
app.post("/fx/understand_and_act", async (req, res) => {
  ...
  try {
    ...
    const scored = candidates
      .map((c) => {
        const label = [c.txt, c.aria, c.title, c.placeholder, c.nameAttr]
          .filter(Boolean)
          .join(" ");
        return {
          ...c,
          _score:
            simpleScore(instruction, label) + (c.rect.y < 700 ? 10 : 0),
        };
      })
...
`.trim();
          ...
          content: [
            {
              ...
              text:
                ...
                top
                  .map(
                    (c) =>
                      `[${c.i}] role=${c.role || c.tag} y=${Math.round(
                        c.rect.y
                      )} label="${[c.txt, c.aria, c.title, c.placeholder]
                        .filter(Boolean)
                        .join(" ")
                        .slice(0, 140)}" href="${c.href || ""}"`
                  )
                  .join("\n"),
            },
          ],
    ...
    try {
      ...
      return res.status(500).json({ error: "Planner returned invalid JSON." });
    }
    ...
    if (plan.intent === "click" && Number.isInteger(plan.target_index)) {
      ...
      if (!target)
        return res
          .status(404)
          .json({ error: "Planner chose a non-existent target", plan });

      const handle = handles[target.i];
      if (!handle)
        return res
          .status(404)
          .json({ error: "Element handle not available", plan });
      ...
      if (!text)
        return res
          .status(400)
          .json({ error: "Planner did not provide text_to_type", plan });
      ...
      const text = await fxPage.evaluate(
      ...
      const qa = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "Answer using only the provided page text. If unsure, say so.",
          },
          {
            role: "user",
      ...
      messages: [
        {
          role: "system",
          content:
            "Extract the smallest accurate snippet that answers the user's query from PAGE_TEXT. If not present, say 'Not found'.",
        },
...
If nothing matches, return {"x":-1,"y":-1,"w":0,"h":0,"label":"not found"}.
      ...
      messages: [
        {
          role: "system",
          content:
            "You locate on-screen UI elements from images and return precise pixel coordinates.",
        },
        {
          role: "user",
          content: [
    ...
    let box;
    try {
      box = JSON.parse(completion.choices[0].message.content || "{}");
    } catch {
      return res
        .status(500)
        .json({ error: "Vision response was not valid JSON." });
    }

    if (!box || box.x < 0 || box.y < 0)
      return res.status(404).json({ error: "Target not found", box });
...
app.listen(PORT, () => {
  ...
  console.log(
    `Try: curl -X POST http://localhost:${PORT}/fx/understand_and_act -H 'Content-Type: application/json' -d '{"instruction":"Open the pricing page"}'`
  );
});
